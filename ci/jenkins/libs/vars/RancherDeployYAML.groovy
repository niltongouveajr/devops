...
def call(){
     try {
        ...
        throw new RuntimeException("Please ensure the " +
            ...
            "'env.RancherDockerInternalPort' and " +
            "'env.RancherYAMLFile' " +
    ...
    sh "kubectl create -f ${RancherYAMLFile} --namespace=${RancherNamespace}"
    sh "kubectl expose deployment/${RancherDeployment} --type=\"NodePort\" --port ${RancherDockerInternalPort} --namespace=${RancherNamespace} || true"
    script {
        ...
        summary.appendText("<a href=\"${RancherService}\">VNT.DevOps on Rancher</a>", false)
    }
}
