...
ANSIBLE_LOCATION="/srv/ansible/server_provising"
ANSIBLE_INVENTORY_FILE="servers"
...
echo -e "\nCommand: ${cyan}ansible-playbook ${ANSIBLE_LOCATION}/${ANSIBLE_PLAYBOOK}.yml -i ${ANSIBLE_LOCATION}/inventories/${ANSIBLE_INVENTORY_FILE} --limit \"${ANSIBLE_HOSTS_GROUPS}\" --tags \"${ANSIBLE_ROLES_TAGS}\"${normal}\n" | tee ${ANSIBLE_LOCATION}/logs/ansible_run.${ANSIBLE_LOGS_DATE}.log
...
ansible-playbook ${ANSIBLE_LOCATION}/${ANSIBLE_PLAYBOOK}.yml -i ${ANSIBLE_LOCATION}/inventories/${ANSIBLE_INVENTORY_FILE} --limit "${ANSIBLE_HOSTS_GROUPS}" --tags "${ANSIBLE_ROLES_TAGS}" 2>&1 | tee -a ${ANSIBLE_LOCATION}/logs/ansible_run.${ANSIBLE_LOGS_DATE}.log
