...
def call() {
    ...
    if (isUnix()) {
        sh 'wget -q https://devops.local/downloads/others/email/logo_header.png -O logo_header.png --no-check-certificate'
        sh 'wget -q https://devops.local/downloads/others/email/logo_footer.png -O logo_footer.png --no-check-certificate'
    } else {
        EnvVarsDefaultWin()
        bat 'wget -q https://devops.local/downloads/others/email/logo_header.png -O logo_header.png --no-check-certificate'
        bat 'wget -q https://devops.local/downloads/others/email/logo_footer.png -O logo_footer.png --no-check-certificate'
    }
                ...
                <tr>
                    <td>
                        <p style="font-family: verdana, helvetica, sans-serif; margin-top: 0px;">
                            ...
                            <br>  
                            <br>
                            A new delivery for <b><span style="color:#ED1B66">${env.JOB_NAME}</span></b> has been generated. More details about the new delivery <b><span style="color:#ED1B66">${TagName}</span></b> can be found at:
                            <br>  
                            <br>
...
"""
    emailext (
        ...
        subject: "[DevOps] ${env.JOB_NAME} (#${env.BUILD_NUMBER}) ~ New Delivery! ",
        body: "${EmailContent}",
        mimeType: 'text/html',
        recipientProviders: [
